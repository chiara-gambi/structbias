...
#Load in data
setwd("~/OneDrive - Cardiff University/Cardiff/research/Pred&Learn/structure_pred/data/all_pilot")
pre_data<-read.table("pre-test-all-pilot.txt",header = T)
post_data<-read.table("post-test-all-pilot.txt",header= T)
...
ggplot(all_data.mean,aes(x=condition,y=accuracy.mean,col=phase))+
...
Summary. 
  ...
  * interaction between sentence type and trial animacy -- the advantage for AI over AA is smaller if training == DO
...
m.tp.ANCOVA.int<-glmer(`accuracy.Post-test`~PC*SCC*ACC*TACC+(1|subject_id)+(1|verb),data=all.tp.w, family="binomial", glmerControl(optimizer = "bobyqa"))
#summary(m.tp.ANCOVA.int)

summary(m.tp.ANCOVA.int)$call
kable(data.frame(coef(summary(m.tp.ANCOVA.int))),caption = "Post-test accuracy, controlling for Pre-test accuracy - Model")
...
Summary for AA
  ...
  * main effect of pre-test accuracy -- higher accuracy in post-test if higher accuracy in pre-test
  * main effect of sentence_type -- higher accuracy for training == D0
...
```{r AAvs.AI}
m.tp.ANCOVA.int.AA<-glmer(`accuracy.Post-test`~PC*SCC*ACC+(1|subject_id)+(1|verb),data=subset(all.tp.w,trial_animacy=="AA"), family="binomial", glmerControl(optimizer = "bobyqa"))
#summary(m.tp.ANCOVA.int.AA)

summary(m.tp.ANCOVA.int.AA)$call
kable(data.frame(coef(summary(m.tp.ANCOVA.int.AA))),caption = "Post-test accuracy, controlling for Pre-test accuracy - Animate-Animate test trials only")

m.tp.ANCOVA.int.AI<-glmer(`accuracy.Post-test`~PC*SCC*ACC+(1|subject_id)+(1|verb),data=subset(all.tp.w,trial_animacy=="AI"), family="binomial", glmerControl(optimizer = "bobyqa"))
#summary(m.tp.ANCOVA.int.AI)

summary(m.tp.ANCOVA.int.AI)$call
kable(data.frame(coef(summary(m.tp.ANCOVA.int.AI))),caption = "Post-test accuracy, controlling for Pre-test accuracy - Animate-Inanimate test trials only")
